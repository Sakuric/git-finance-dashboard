...
import { searchStock, getWatchlist, getMarketIndices, getMarketOverview, getIndexKLine, getLatestAdvice } from '@/api/stock'
    ...
    if (res?.code === 200 && res.data) {
      ...
      try {
        ...
        if (recommendations.length > 0) {
          ...
          aiAdvice.value.content = `根据您的风险偏好，当前建议关注 ${firstRec.name || '相关'} (${firstRec.code || ''})。${firstRec.thesis ? firstRec.thesis.substring(0, 50) + '...' : ''}`
        } else {
